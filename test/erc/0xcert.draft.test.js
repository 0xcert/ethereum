...
contract('Xcert', function(accounts) {
  ...
  describe('should test minting and deed count', function () {

    it("should return 0 minted deeds", async function() {
      var count = await instance.countOfDeeds.call();
      assert.equal(count.toNumber(), 0, "There should be 0 minted deed");
    });

    it("should return correct count of deeds after mint", async function() {
      await instance.mint.sendTransaction(accounts[0],id1,"url");
      var count = await instance.countOfDeeds.call();
      assert.equal(count.toNumber(), 1, "There should be 1 minted deed");
    });

    it("should throw error when trying to mint 2 deeds with the same hash", async function() {
    ...
    it("should throw error trying to mint deed with empty hash", async function() {
      await assertRevert(instance.mint.sendTransaction(accounts[0],"",""));
      var count = await instance.countOfDeeds.call();
      assert.equal(count.toNumber(), 2, "There should be 2 minted deed");
    });

    it("should throw error trying to ming deed to 0x0 address ", async function() {
      await assertRevert(instance.mint.sendTransaction("0",id3,""));
      var count = await instance.countOfDeeds.call();
      assert.equal(count.toNumber(), 2, "There should be 2 minted deed");
    });

    it("should throw error trying to mint deed from non owner address", async function() {
    ...
    it("should throw error trying to mint deed with uri length more then 2083 chars", async function() {
 ...
 describe('should test deed ownership', function () {
   ...
   it("should throw error trying to get count of deeds owned by 0x0 address", async function() {
      await assertRevert(instance.countOfDeedsByOwner.call("0"));
    });

   it("should find the correct owner of deed id 3", async function() {
      var address = await instance.ownerOf.call(id3);
      assert.equal(address, accounts[1], "Owner should be accounts[1]");
    });

    it("should throw error trying to find owner od none existant deed id", async function() {
      await assertRevert(instance.ownerOf.call(id4));
    });

  });

  describe('should test approval and takeOwnership', function () {

    it("should correctly approve account[1]", async function() {
    ...
    it("should correctly cancel approval of account[1]", async function() {
      await instance.approve.sendTransaction(0,id2);
      var address = await instance.approvedFor.call(id2);
      assert.equal(address, 0, "Should be 0");
    });

    it("should return 0x0 address for check approval of none existant deed id", async function() {
      var address = await instance.approvedFor.call(id4);
      assert.equal(address, 0, "Should be 0");
    });

    it("should correctly approve and take ownership of deed id 2 from account[0] to account[1]", async function() {
      await instance.approve.sendTransaction(accounts[1],id2);
    ...
    it("should throw error trying to take ownership that is not approved for", async function() {
    ...
    it("should throw error trying to take ownership of deed that approval was granted but then canceled", async function() {
    ...
    it("should throw error trying to approve deed id that we are not the owner of", async function() {
  ...
  describe('should test metadata', function () {

   it("should return the correct issuer name", async function() {
      var name = await instance.name.call();
      assert.equal(name, "test", "Should be test");
    });

   it("should return the correct issuer symbol", async function() {
      var symbol = await instance.symbol.call();
      assert.equal(symbol, "TST", "Should be TST");
    });

   it("should return the correct deed id 2 url", async function() {
      var deedUri = await instance.deedUri.call(id2);
      assert.equal(deedUri, "url2", "Should be url2");
    });

   it("should throw error trying to get uri of none existant deed id", async function() {
  ...
  describe('should test deedOfOwnerByIndex', function () {

    it("should return the correct deed id for account[1] index 1", async function() {
      var xcertId = await instance.deedOfOwnerByIndex.call(accounts[1], 0);
      assert.equal(util.intToHex(xcertId), id3, "Should be 3");
    });

   it("should throw error trying to get id of none existant index", async function() {
      await assertRevert(instance.deedOfOwnerByIndex.call(accounts[1], 5));
    });

  });

  describe('should test burning', function () {

   it("should destory deed id 1", async function() {
   ...
   it("should throw error trying to destory already destroyed deed id 1", async function() {
    ...
    it("should throw error trying to destory deed you are not the owner of", async function() {
  ...
  describe('should test deedsOf', function () {

    it("should return the correct array of deed ids of accounts[1]", async function() {
    ...
    it("should return the correct array size of accounts[0]", async function() {
      var deedIds = await instance.deedsOf.call(accounts[0]);
      assert.equal(deedIds.length, 0, "Should be 0");
    });

    it("should throw error trying to get array of deeds for 0x0 adress", async function() {
      await assertRevert(instance.deedsOf.call(0));
    });

    it("should return the correct array size for account[3]", async function() {
  ...
  describe('should test deed transfer', function () {

    it("should correctly transfer deed ownership", async function() {
    ...
    it("should throw error trying to transfer to a 0x0 account", async function() {
      await assertRevert(instance.transfer.sendTransaction(0, id2));
    });

    it("should throw error trying to transfer an none existant deed", async function() {
      await assertRevert(instance.transfer.sendTransaction(accounts[1], id4));
    });

    it("should throw error trying to transfer deed you are not an owner of", async function() {
      await assertRevert(instance.transfer.sendTransaction(accounts[1], id3));
    });

   it("should throw error trying to transfer deed to yourself", async function() {
